---
title: "Ongelijke verdeling van misdaad tussen buurten in Nederlandse gemeenten"
bibliography: "concentration.bib"
...
municipality_gini_df <- 
  ...
  dplyr::summarize(
    ...
    gini_crime_pop_weighted = REAT::gini2(x = crime, weighting = population, coefnorm = TRUE),
...
# Histogram of crime Gini weighted by population across all municipalities ----- 
n_municipalities <- 
  municipality_gini_df |>
  nrow()

gini_crime_popweighted_histogram_all_ggp <-
  municipality_gini_df |>
  ggplot() +
  geom_histogram(aes(gini_crime_pop_weighted), 
                 color = "black", 
                 fill = CRIME_COLOR,
                 binwidth = .05) +
  scale_x_continuous(breaks = seq(.2, .9, .1)) +
  xlab("Gini coëfficiënt") +
  ylab("Aantal gemeenten") +
  ggtitle(paste0("Verdeling Gini coëfficiënt misdaad gewogen naar bevolkingsomvang, alle ", 
                 n_municipalities, 
                 " gemeenten"))
gini_crime_popweighted_histogram_all_ggp
ggsave_svg(ggp = gini_crime_popweighted_histogram_all_ggp, output = OUTPUT)

# Histogram of crime Gini weighted by population across 3_n municipalities  ----
n_municipalities_3p <-
  municipality_gini_df |>
  filter(n_nhoods > 2) |>
  nrow()

gini_crime_popweighted_histogram_min3nhoods_ggp <-
  municipality_gini_df |>
  filter(n_nhoods > 2) |>
  ggplot() +
  geom_histogram(aes(gini_crime_pop_weighted), 
  ...
  xlab("Gini coëfficiënt") +
  ylab("Aantal gemeenten") +
  ggtitle(paste0("Verdeling Gini coëfficiënt misdaad gewogen naar bevolkingsomvang, ", 
                 n_municipalities_3p, 
                 " gemeenten"))
gini_crime_popweighted_histogram_min3nhoods_ggp
ggsave_svg(ggp = gini_crime_popweighted_histogram_min3nhoods_ggp, output = OUTPUT)

# Calculate correlation between weighted and unweighted crime Gini
gini_unweighted_weighted_cor <-
  municipality_gini_df |> 
  filter(n_nhoods > 2) |> 
  select(starts_with("gini_crime")) |> 
  cor()
gini_unweighted_weighted_cor <- gini_unweighted_weighted_cor[2,1]

# Scatterplot of weighted versus unweighted crime Gini -------------------------
gini_unweighted_weighted_ggp <-
  municipality_gini_df |> 
  filter(n_nhoods > 2) |> 
  ggplot() + 
  geom_point(aes(x= gini_crime_unweighted, 
                 y = gini_crime_pop_weighted)) +
  scale_x_continuous(breaks = seq(.2, .9, .1)) +
  xlab("Ongewogen Gini coëfficiënt") +
  ylab("Gewogen Gini coëfficiënt") +
  ggtitle(paste0("Gewogen en ongewogen Gini coëfficiënt. R = ",
                   round(gini_unweighted_weighted_cor, digits = 2)
  ))
gini_unweighted_weighted_ggp   
ggsave_svg(ggp = gini_unweighted_weighted_ggp, output = OUTPUT)
...
In 1906 stelde de Italiaanse wiskundige Vilfredo Pareto vast dat 80 procent van de bezittingen in Italië in handen was van de rijkste 20 procent van de bevolking \[REFERENTIE\]. Dit soort scheve verdelingen, vaak aangeduid als '80-20 regel' zijn op veel verschijnselen van toepassing. \[VOORBEELDEN, GRAAG GEFUNDEERD\].

Ook misdaad is ongelijk verdeeld. Zo is bekend dat een klein percentage daders ('veelplegers', 'notoire recidivisten' of 'draaideurcriminelen') verantwoordelijk is voor een groot percentage van alle misdrijven \[REFERENTIE\].

Deze *Staat van de misdaad* gaat over buurtverschillen in misdaad. We onderzoeken in hoeverre binnen Nederlandse gemeenten de geregistreerde misdaad ongelijk verdeeld is tussen de buurten in die gemeenten. Als misdrijven in alle buurten van een gemeente ongeveer even vaak voorkomen is de verdeling gelijk. Als de meeste misdrijven in een of in enkele buurten plaatsvinden is de verdeling ongelijk.

Ongelijke verdelingen laten zich goed in beeld brengen met behulp van de zogenoemde Lorenz-curve, vernoemd naar de Amerikaanse econoom Max Otto Lorenz. Figuur 1 is een voorbeeld een Lorenz-curve. In dit geval betreft het de verdeling van misdrijven over de 42 buurten van de willekeurig gekozen Nederlandse gemeente Katwijk. De horizontale as van de grafiek geeft het cumulatieve percentage buurten weer, in aflopende volgorde gesorteerd op basis van het aantal in 2023 geregistreerde misdrijven. De verticale as geeft het cumulatieve percentage weer van de totale misdaad dat in de betreffende buurten plaatsvond.
...
To also disable the printing of output:
...
You should not see anything between this line and the line saying "To also disable the printing of output:".
